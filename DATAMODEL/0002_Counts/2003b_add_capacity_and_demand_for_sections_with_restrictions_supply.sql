...
DECLARE
	...
	relevant_restriction_in_section RECORD;
...
BEGIN
    ...
    ***/
	...
	/* Now consider restrictions for supply */
	
	Supply_Capacity = 0.0;
	Capacity := 0;
	
	FOR relevant_restriction_in_section IN
		SELECT "GeometryID", "RestrictionTypeID", "Capacity"
		FROM mhtc_operations."Supply"
        WHERE "Demand_GeometryID" = NEW."GeometryID"

    LOOP

		RestrictionTypeID = relevant_restriction_in_section."RestrictionTypeID";
		...
		IF (RestrictionTypeID = 201 OR RestrictionTypeID = 221 OR RestrictionTypeID = 224 OR   -- SYLs
			...
			RAISE NOTICE '--- checking SYL capacity for (%); survey (%) ', relevant_restriction_in_section."GeometryID", NEW."SurveyID";
			...
			IF check_exists THEN
				...
				WHERE s."GeometryID" = relevant_restriction_in_section."GeometryID"
				AND s."NoWaitingTimeID" = t."TimePeriodID"
				AND t."SurveyID" = NEW."SurveyID";

				IF controlled THEN
					RAISE NOTICE '*****--- capacity is  0 ...';
					--Supply_Capacity = 0.0;
					CONTINUE;
				END IF;
		...
		IF check_dual_restrictions_exists THEN
			...
			AND d."LinkedTo" = relevant_restriction_in_section."GeometryID";
			...
			IF primary_geometry_id IS NOT NULL THEN
				...
				RAISE NOTICE '*****--- % Primary restriction. Checking time period % ...', relevant_restriction_in_section."GeometryID", time_period_id;
			...
			FROM mhtc_operations."Supply" s, mhtc_operations."DualRestrictions" d
			WHERE s."GeometryID" = d."GeometryID"
			AND d."GeometryID" = relevant_restriction_in_section."GeometryID";

			IF secondary_geometry_id IS NOT NULL THEN

				-- restriction is "secondary". Need to check whether or not it is active
				RAISE NOTICE '*****--- % Secondary restriction. Checking time period % ...', relevant_restriction_in_section."GeometryID", time_period_id;
		...
		Supply_Capacity = Supply_Capacity + relevant_restriction_in_section."Capacity";
